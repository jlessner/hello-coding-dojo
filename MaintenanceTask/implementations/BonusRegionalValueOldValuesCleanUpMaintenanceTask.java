...
public class BonusRegionalValueOldValuesCleanUpMaintenanceTask extends CleanUpMaintenanceTask {
    ...
    public void setup() throws MaintenanceTaskSetupFailedException {
        super.setup();
        this.pivot = DateTool.getEndOfDay(DateTool.addMonths(DateTool.getCurrent(), -12));
    }

    private static final String SQL_WHERE_PART =
            " WHERE " + BonusRegionalValueDAO.column_periodEnd + " IS NOT NULL"
            + " AND " + BonusRegionalValueDAO.column_periodEnd + " <= ?"
            + " AND " + BonusRegionalValueDAO.column_value + " > ?";
    ...
    protected Long countValuesToDelete() throws Exception {
        if(this.pivot != null) {
            return CommandProcessor.process(c -> {
                ...
                stmt.setTimestamp(1, new Timestamp(this.pivot.getTime()));
